...
function getTaskListMetadata(bodyValue) {
  const items = parseTaskListItems(bodyValue);
  const completed = items.filter((item) => item.isCompleted).length;
  const dates = (field, openOnly) => [...new Set(items
    .filter((item) => item[field] && (!openOnly || !item.isCompleted))
    .map((item) => item[field]))].sort();
  return {
    taskSummary: { total: items.length, completed, remaining: items.length - completed },
    taskStartDates: dates('start', false),
    incompleteTaskStartDates: dates('start', true),
    taskDueDates: dates('due', false),
    incompleteTaskDueDates: dates('due', true),
  };
}
