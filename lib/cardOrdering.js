...
const { getIncompleteTaskListDueDates } = require('./taskList');
...
function getCardDueDateForOrdering(card) {
  const cardDueDate = String(card && card.frontmatter && card.frontmatter.due || '').trim();
  if (cardDueDate) {
    return cardDueDate;
  }

  const taskDueDates = getIncompleteTaskListDueDates(card && card.body || '');
  return taskDueDates[0] || '';
}

async function reorderCardFilesInListByDueDate(listPath, options = {}) {
  const resolvedListPath = path.resolve(listPath);
  const readCard = typeof options.readCard === 'function'
    ? options.readCard
    : cardFrontmatter.readCard;
  const currentCardFiles = await listCardFiles(resolvedListPath);
  const orderedCards = await Promise.all(currentCardFiles.map(async (cardFile, originalIndex) => {
    const sourcePath = path.join(resolvedListPath, cardFile);
    const card = await readCard(sourcePath);

    return {
      sourcePath,
      dueDate: getCardDueDateForOrdering(card),
      originalIndex,
    };
  }));
  ...
  const dueDateBySourcePath = new Map(
    orderedCards.map((entry) => [entry.sourcePath, entry.dueDate]),
  );
  const reorderedCards = await reorderCardFilesInList(
    resolvedListPath,
    orderedCards.map((entry) => entry.sourcePath),
  );

  return {
    cards: reorderedCards.map((entry) => ({
      ...entry,
      dueDate: dueDateBySourcePath.get(entry.sourcePath) || '',
    })),
    changed: reorderedCards.some((entry) => entry.sourcePath !== entry.cardPath),
    datedCardCount: orderedCards.filter((entry) => entry.dueDate).length,
  };
}
