...
const { archiveCard: archiveCardOnDisk } = require('./archive');
  ...
  switch (name) {
    ...
    case 'signboard.create_card': {
      ...
      await cardFrontmatter.writeCard(cardPath, {
        frontmatter: prepareNewCardFrontmatter(frontmatter),
    ...
    case 'signboard.import_tasksmd': {
      assertWriteAllowed(config, 'signboard.import_tasksmd');
