...
const { renameManagedDirectory } = require('./directoryRename');
const { assertAllowedPath, assertAllowedTree } = require('./allowedPaths');
const { normalizeSignboardCardFrontmatter } = require('./obsidianIntegration');
const { parseLocalIsoDate } = require('../shared/localDate');
...
async function assertInsideAllowedRoots(config, candidatePath, fieldName) {
  if (!config.allowedRoots.length) {
    throw new Error(`${fieldName} requires SIGNBOARD_MCP_ALLOWED_ROOTS to be configured.`);
  }
  await assertAllowedPath(config.allowedRoots, candidatePath, fieldName);
}

function normalizeCardDateInput(value, fieldName) {
  if (value == null || String(value).trim() === '') return undefined;
  const normalized = String(value).trim();
  if (!parseLocalIsoDate(normalized)) throw new Error(`${fieldName} must be a valid YYYY-MM-DD date.`);
  return normalized;
}
...
async function resolveBoardRoot(config, boardRootInput) {
  ...
  await assertInsideAllowedRoots(config, resolvedBoardRoot, 'boardRoot');

  await assertInsideAllowedRoots(config, path.join(resolvedBoardRoot, 'board-settings.md'), 'board settings');
  await assertInsideAllowedRoots(config, path.join(resolvedBoardRoot, 'labels.md'), 'legacy board settings');
  await assertInsideAllowedRoots(config, path.join(resolvedBoardRoot, ARCHIVE_DIRECTORY_NAME), 'archive');
...
async function resolveExistingDirectoryPath(config, directoryPathInput, fieldName) {
  ...
  await assertInsideAllowedRoots(config, resolvedDirectoryPath, fieldName);
...
async function resolveExistingFilePath(config, filePathInput, fieldName) {
  ...
  await assertInsideAllowedRoots(config, resolvedFilePath, fieldName);
...
async function resolveExistingPath(config, pathInput, fieldName) {
  ...
  await assertInsideAllowedRoots(config, resolvedPath, fieldName);
  return resolvedPath;
}
...
async function resolveExistingImportSourcePath(config, sourcePathInput, fieldName) {
  ...
  await assertInsideAllowedRoots(config, resolvedSourcePath, fieldName);
...
async function validateDestinationBoardRoot(config, boardRoot, destinationBoardRoot, fieldName) {
  ...
  await assertInsideAllowedRoots(config, destinationBoardRoot, fieldName);
  ...
  await assertInsideAllowedRoots(config, destinationParent, 'destination parent directory');
...
async function resolveListPath(config, boardRootInput, listNameInput) {
  ...
  await assertInsideAllowedRoots(config, listPath, 'listName');
...
async function resolveCardPath(config, boardRootInput, listNameInput, cardFileInput) {
  ...
  await assertInsideAllowedRoots(config, cardPath, 'cardFile');
...
async function handleToolCall(config, name, args = {}) {
  ...
  switch (toolName) {
    ...
    case 'signboard.create_card': {
      ...
      if (Object.prototype.hasOwnProperty.call(args, 'start')) {
        if (args.start == null || String(args.start).trim() === '') {
          ...
          frontmatter.start = normalizeCardDateInput(args.start, 'start');
        }
      }
      ...
      if (Object.prototype.hasOwnProperty.call(args, 'due')) {
        if (args.due == null || String(args.due).trim() === '') {
          ...
          frontmatter.due = normalizeCardDateInput(args.due, 'due');
        }
      }
      ...
      const nextFrontmatter = normalizeSignboardCardFrontmatter({ boardRoot, cardPath, frontmatter: prepareNewCardFrontmatter(frontmatter) });
    ...
    case 'signboard.update_card': {
      ...
      const nextFrontmatter = normalizeSignboardCardFrontmatter({ boardRoot, cardPath, frontmatter: current.frontmatter });
      ...
      if (Object.prototype.hasOwnProperty.call(args, 'due')) {
        if (args.due == null || String(args.due).trim() === '') {
          ...
          nextFrontmatter.due = normalizeCardDateInput(args.due, 'due');
        }
      }
      ...
      if (Object.prototype.hasOwnProperty.call(args, 'start')) {
        if (args.start == null || String(args.start).trim() === '') {
          ...
          nextFrontmatter.start = normalizeCardDateInput(args.start, 'start');
        }
      }
    ...
    case 'signboard.duplicate_card': {
      ...
      await assertInsideAllowedRoots(config, targetListPath, 'target list');
      ...
      const duplicatedFrontmatter = normalizeSignboardCardFrontmatter({
        boardRoot: source.boardRoot, cardPath: duplicatedCardPath,
        frontmatter: prepareNewCardFrontmatter({ ...sourceCard.frontmatter, title: nextTitle, labels }),
      });
    ...
    case 'signboard.archive_card': {
      ...
      await assertAllowedTree(config.allowedRoots, source.boardRoot);
    ...
    case 'signboard.archive_list': {
      ...
      await assertAllowedTree(config.allowedRoots, boardRoot);
    ...
    case 'signboard.list_archive_entries': {
      const boardRoot = await resolveBoardRoot(config, args.boardRoot);
      if (await pathExists(path.join(boardRoot, ARCHIVE_DIRECTORY_NAME))) {
        await assertAllowedTree(config.allowedRoots, path.join(boardRoot, ARCHIVE_DIRECTORY_NAME));
      }
    ...
    case 'signboard.read_archive_entry': {
      ...
      await assertAllowedTree(config.allowedRoots, entryPath);
    ...
    case 'signboard.move_card': {
      ...
      await assertInsideAllowedRoots(config, sourcePath, 'source card');
      await assertInsideAllowedRoots(config, destinationPath, 'destination card');
      ...
      let nextFrontmatter = movedCard.frontmatter;
      if (fromListName !== toListName && fromListName !== ARCHIVE_DIRECTORY_NAME && toListName !== ARCHIVE_DIRECTORY_NAME) {
        nextFrontmatter = applyCardListMove(nextFrontmatter, {
      ...
      await cardFrontmatter.writeCard(finalDestinationPath, {
        frontmatter: normalizeSignboardCardFrontmatter({ boardRoot, cardPath: finalDestinationPath, frontmatter: nextFrontmatter }),
        body: movedCard.body,
      });
    ...
    case 'signboard.rename_board': {
      ...
      if (newBoardRoot !== boardRoot) {
        ...
        await assertAllowedTree(config.allowedRoots, boardRoot);
        await renameManagedDirectory(boardRoot, boardRoot, newBoardRoot);
      }
    ...
    case 'signboard.import_obsidian': {
      ...
      await assertAllowedTree(config.allowedRoots, boardRoot);
      for (const sourcePath of sourcePaths) await assertAllowedTree(config.allowedRoots, sourcePath);
    ...
    case 'signboard.import_tasksmd': {
      ...
      await assertAllowedTree(config.allowedRoots, boardRoot);
      await assertAllowedTree(config.allowedRoots, sourcePath);
