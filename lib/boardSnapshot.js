...
const { mapFileReads, withFileReadLimit } = require('./fileReadQueue');
...
async function listBoardDirectories(boardRoot, options = {}) {
  ...
  const entries = await withFileReadLimit(() => fs.readdir(boardRoot, { withFileTypes: true }));
  return entries
...
async function listMarkdownCardFileNames(listPath) {
  const entries = await withFileReadLimit(() => fs.readdir(listPath, { withFileTypes: true }));
  return entries
...
async function readSnapshotList(boardRoot, listName, errors, options = {}) {
  ...
  const cards = (await mapFileReads(
    cardNames, (cardName) => readSnapshotCard(path.join(listPath, cardName), cardName, errors, options),
  )).filter(Boolean);
