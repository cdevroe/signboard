...
async function renameList(boardRoot, listRef, newName) {
  ...
  await renameManagedDirectory(resolvedBoardRoot, listEntry.path, nextPath);
...
async function loadCardRecord(boardRoot, listEntry, fileName) {
  ...
  const taskMetadata = getTaskListMetadata(card.body);
...
async function listCards(boardRoot, options = {}) {
  ...
  for (const listEntry of selectedLists) {
    ...
    const loaded = await mapFileReads(
      cardNames, (fileName) => loadCardRecord(resolvedBoardRoot, listEntry, fileName)
    );
...
async function resolveCard(boardRoot, options = {}) {
  ...
  // Exact filenames take precedence over title/partial matches. Resolve against
  // the directory inventory to preserve case-insensitive ambiguity detection.
  if (options.listRef && cardRef.toLowerCase().endsWith('.md')) {
    const list = await resolveList(resolvedBoardRoot, options.listRef, { includeArchive, withCardCounts: false });
    const entries = await fs.readdir(list.path, { withFileTypes: true });
    const matches = entries.filter((entry) => entry.isFile() && entry.name.endsWith('.md')
      && normalizeMatchValue(entry.name) === normalizeMatchValue(cardRef));
    if (matches.length > 1) throw new Error(`Ambiguous card reference "${cardRef}".`);
    if (matches.length === 1) {
      const record = await loadCardRecord(resolvedBoardRoot, list, matches[0].name);
      const settings = await boardLabels.readBoardSettings(resolvedBoardRoot, { ensureFile: false });
      const labels = new Map(settings.labels.map((label) => [label.id, label.name]));
      return { ...record, labelNames: record.labels.map((id) => labels.get(id) || id)
        .sort((left, right) => listSortCollator.compare(left, right)) };
    }
  }
...
async function editCard(boardRoot, options = {}) {
  ...
  if (options.dryRun === true) {
    ...
    const targetFilePath = path.join(targetList.path, cardRecord.fileName);
    const normalizedFrontmatter = obsidianIntegration.normalizeSignboardCardFrontmatter({
      boardRoot: resolvedBoardRoot,
      cardPath: targetFilePath,
    ...
    return {
      ...
      title: String(normalizedFrontmatter.title || '').trim() || cardRecord.title,
      start: String(normalizedFrontmatter.start || ''),
      due: String(normalizedFrontmatter.due || ''),
      labels: Array.isArray(normalizedFrontmatter.labels) ? normalizedFrontmatter.labels : [],
      labelNames: await labelNamesForIds(resolvedBoardRoot, normalizedFrontmatter.labels),
      frontmatter: normalizedFrontmatter,
      body: nextBody,
      timestamps: {
        ...resolveCardTimestamps(normalizedFrontmatter),
  ...
  await cardFrontmatter.writeCard(cardRecord.filePath, {
    frontmatter: obsidianIntegration.normalizeSignboardCardFrontmatter({
      boardRoot: resolvedBoardRoot,
      cardPath: cardRecord.filePath,
      frontmatter: nextFrontmatter,
    }),
    body: nextBody,
  });
