...
const { getBuildInfo, formatBuildInfo } = require('./buildInfo');
...
async function runCardsCommand(action, positionals, options, context) {
  ...
  if (action === 'create') {
    assertAllowedOptions(
      options,
      ['list', 'title', 'from-card', 'from-list', 'body', 'body-file', 'start', 'due', 'label', 'remove-label', 'clear-labels', 'dry-run', 'json', 'board'],
...
async function runCli(argv, context = {}) {
  ...
  if (options.version) {
    assertAllowedOptions(options, ['version', 'json'], `Usage: ${commandName} --version [--json]`);
    assertNoExtraPositionals(positionals, 0, `Usage: ${commandName} --version [--json]`);
    const info = getBuildInfo();
    stdout.write(`${options.json ? JSON.stringify(info, null, 2) : formatBuildInfo(info)}\n`);
    return 0;
  }
