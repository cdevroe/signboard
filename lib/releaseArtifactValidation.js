...
function inspectPacmanPackageBuffer(buffer) {
  const source = Buffer.isBuffer(buffer) ? buffer : Buffer.alloc(0);
  const hasKnownCompression =
    source.subarray(0, ZSTD_MAGIC.length).equals(ZSTD_MAGIC) ||
    source.subarray(0, GZIP_MAGIC.length).equals(GZIP_MAGIC) ||
    source.subarray(0, XZ_MAGIC.length).equals(XZ_MAGIC);
  const hasUncompressedTarHeader = source.length >= 262 && source.toString('ascii', 257, 262) === 'ustar';
  return {
    valid: hasKnownCompression || hasUncompressedTarHeader,
    errors: hasKnownCompression || hasUncompressedTarHeader
      ? []
      : ['File is not a recognized compressed or uncompressed Pacman package archive.'],
  };
}

function inspectPacmanPackageFile(filePath) {
  try {
    const fileDescriptor = fs.openSync(filePath, 'r');
    try {
      const fileSize = fs.fstatSync(fileDescriptor).size;
      const prefix = readFileRange(fileDescriptor, 0, Math.min(fileSize, 512));
      return inspectPacmanPackageBuffer(prefix);
    } finally {
      fs.closeSync(fileDescriptor);
    }
  } catch (error) {
