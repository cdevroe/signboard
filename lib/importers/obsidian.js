...
const CARD_BOARD_START_PATTERNS = [
  { type: 'cardboard', pattern: /@(?:start|scheduled)\(([^)]+)\)/i },
  { type: 'dataview', pattern: /\[(?:start|scheduled)::\s*([^\]]+)\]/i },
  { type: 'tasks-start', pattern: /🛫\s*(\d{4}-\d{2}-\d{2})/i },
  { type: 'tasks-scheduled', pattern: /⏳\s*(\d{4}-\d{2}-\d{2})/i },
];
...
function parseStartFromText(text) {
  const source = String(text || '');

  for (const matcher of CARD_BOARD_START_PATTERNS) {
    const match = source.match(matcher.pattern);
    if (!match) {
      continue;
    }
    ...
    return {
      type: matcher.type,
      raw: rawValue,
      start: normalizeIsoDateFromValue(rawValue),
    };
  }
...
function stripConsumedTaskMetadata(text) {
  return String(text || '')
    ...
    .replace(/@due\(([^)]+)\)/gi, '')
    .replace(/@(?:start|scheduled)\(([^)]+)\)/gi, '')
    .replace(/\[due::\s*[^\]]+\]/gi, '')
    .replace(/\[(?:start|scheduled)::\s*[^\]]+\]/gi, '')
...
function parseMarkdownTaskBlocks(rawContent, filePath) {
  ...
  function finalizeCurrentTask() {
    ...
    currentTask.startInfo = parseStartFromText(currentTask.rawLine);
    currentTask.completedInfo = parseCompletedFromText(currentTask.rawLine);
    currentTask.start = currentTask.startInfo && currentTask.startInfo.start
      ? currentTask.startInfo.start
      : '';
...
function parseObsidianKanbanColumns(rawContent, filePath) {
  ...
  return columns.map((column) => ({
    ...
    cards: column.cards.map((card) => {
      ...
      const startInfo = parseStartFromText(card.rawLine);
      const tags = collectInlineTags(card.rawLine);
      return {
        ...card,
        start: startInfo && startInfo.start ? startInfo.start : '',
...
function buildTaskMetadataSection(task, metadata = {}) {
  return buildObsidianMetadataSection({
    ...
    'Original start expression': task.startInfo?.raw || '',
