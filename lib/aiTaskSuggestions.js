...
const SMART_CARD_ACTION_TYPES = new Set(['title', 'tasks', 'paste', 'custom']);
...
function normalizeSmartCardAction(actionDefinition = {}) {
  ...
  const inferredType = id === 'generate-title'
    ? 'title'
    : (id === 'generate-task-list' ? 'tasks' : (id === 'smart-paste' ? 'paste' : 'custom'));
...
function getSmartCardActionOutputInstruction(action, taskCount) {
  if (action.type === 'title') {
    return 'Return JSON only in this shape: {"title":"Improved card title"}.';
  }

  if (action.type === 'tasks') {
    return `Return JSON only in this shape: {"tasks":["Task one","Task two"]}. Return ${taskCount} concrete tasks whenever possible.`;
  }
  ...
  return [
    {
      ...
      content: [
        ...
        getSmartCardActionOutputInstruction(action, taskCount),
      ].join(' '),
    },
    {
      role: 'user',
...
async function runSmartCardActionWithOllama(ollamaSettings = {}, actionDefinition = {}, cardContext = {}, options = {}) {
  ...
  if (action.type === 'title') {
    ...
    result.tasks = extractSuggestedTasksFromContent(content, { maxTasks: taskCount });
  } else {
