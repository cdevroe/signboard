...
function serializeBoardSettings(settings) {
  ...
  const yamlText = yaml.dump(serializable, {
...
async function readBoardSettings(boardRoot, options = {}) {
  ...
  if (ensureFile) {
    const shouldRewrite =
      ...
      !areNotificationSettingsEqual(parsed.notifications, normalized.notifications) ||
      !areTooltipsEnabledEqual(parsed.tooltipsEnabled, normalized.tooltipsEnabled);
...
async function updateBoardSettings(boardRoot, partialSettings = {}) {
  ...
  if ('notifications' in partialSettings) {
    nextSettings.notifications = cloneNotificationSettings(partialSettings.notifications);
  }

  if ('tooltipsEnabled' in partialSettings) {
    nextSettings.tooltipsEnabled = normalizeTooltipsEnabled(partialSettings.tooltipsEnabled);
  }
