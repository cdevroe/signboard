...
const { assertAllowedPath } = require('./allowedPaths');
...
async function buildBoardRecord(candidate, context) {
  ...
  const isAllowed = context.allowedRoots.length === 0
    ? context.allowWhenNoAllowedRoots
    : await assertAllowedPath(context.allowedRoots, boardRoot).then(() => true, () => false);
  return {
