...
- `preload.js` - Renderer bridge (`window.board`, `window.chooser`, `window.electronAPI`).
- `index.html` - App shell, modal markup, and script/style includes.
...
- `app/lists/createListElement.js` - List DOM rendering, rename, card DnD handling.
- `app/board/renderBoard.js` - Whole-board render and list DnD handling.
- `app/board/openBoard.js` - Open/init board directory and starter content.
- `app/modals/closeAllModals.js` - Modal close logic + editor cleanup + rerender.
...
- `app/modals/toggleEditCardModal.js` - Card editor open/save/archive/duplicate logic.
