...
- `app/board/smartBoardActions.js` - Active-board Smart button/popover, Normal/Advanced profile selection, one-off prompt collection, provider dispatch, centered three-dot loading state, independently scrolling report/result body with persistent footer controls, authoritative title-forward/openable referenced cards, per-change review, archive confirmation, report copy, and selective apply flow.
