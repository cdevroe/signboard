...
- `app/board/smartBoardActions.js` - Active-board Smart button/popover, Normal/Advanced profile selection, one-off prompt collection, provider dispatch, centered three-dot loading state, content-sized short results and independently scrolling constrained results with persistent footer controls, authoritative title-forward whole-card references that open the Card Editor, per-change review, archive confirmation, report copy, and selective apply flow.
...
- `lib/aiTaskSuggestions.js` - Native Ollama, LM Studio, OpenAI, Gemini, and Anthropic model-list/generation adapters; action-specific structured-output schemas; Smart Card Action parsing; and Smart Board Action structured result dispatch.
...
- `lib/smartActionSharing.js` - Portable `signboard-smart-action` v1 package normalization, base64url fragment encoding/decoding, share URL generation, and safe export filenames.
...
- `scripts/test-smart-action-sharing.js` - Node assertions for Card/Board package normalization, URL fragment round trips, malformed payload rejection, and safe filenames.
