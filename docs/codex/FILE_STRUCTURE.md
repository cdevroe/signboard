...
- `app/board/smartBoardActions.js` - Active-board Smart button/popover, Normal/Advanced profile selection, one-off prompt collection, provider dispatch, accessible report/result modal, referenced-card display, per-change review, archive confirmation, report copy, and selective apply flow.
