...
- `preload.js` - Thin renderer bridge (`window.board`, `window.chooser`, `window.electronAPI`) that forwards allowed operations to main-process IPC and main-process-triggered renderer events, including board switcher/view/settings/Quick Add/signboard-card-link/signboard-board-link/system-resume events, clipboard text copy, Smart Card Actions, AI provider model inspection, write-only cloud credential set/clear calls, archive browse/read/restore, board snapshot reads, board duplication, Obsidian actions, dropped-file path extraction for linked objects, and transactional card/list reorder plus top-of-list card move calls.
...
- `app/appSettings.js` - Renderer app-settings state, app-wide General/Notifications/Smart Actions controls, independent Normal/Advanced provider/model editors for Ollama/LM Studio/OpenAI/Gemini/Anthropic, cloud key set/clear UI and credential-presence state, drag-reorderable accordion Smart Card Action prompt/target rows, tooltip/notification/Quick Add global shortcut/AI assistance/External Published Calendar persistence scheduling, and one-time migration from legacy board settings; shared defaults/normalizers come from `shared/appSettingsSchema.js`.
...
- `shared/appSettingsSchema.js` - Pure app-settings defaults and normalizers shared by the main process and renderer bundle; the single source for Normal/Advanced profiles, Ollama/LM Studio/OpenAI/Gemini/Anthropic provider defaults, built-in Smart Card Action prompts, targets, saved action order, and legacy one-profile migration.
...
- `lib/aiTaskSuggestions.js` - Native Ollama `/api/tags` + `/api/chat`, LM Studio `/v1/models` + `/v1/chat/completions`, OpenAI, Gemini, and Anthropic model-list/generation adapters, action-specific JSON schemas, response parsing, Smart Card Action output parsing including label references, due dates, linked-object attachment suggestions, read-only card-question answers, checklist task cleanup, and card-context prompt helpers for Card Editor Smart Card Actions.
...
- `ai-credentials.json` (runtime, Electron `userData`) - OS-encrypted OpenAI, Gemini, and Anthropic API key values; never contains plaintext keys or app settings.
