...
The Board tab starts with Ask the Board, Board Brief, Find Quick Wins, Create Cards, Clean Up Board, and Label Board. Open an active board and choose `Smart` in the header to run one. While an action runs, the result window centers an animated three-dot progress indicator. Report actions are read-only; their footer remains available while long report content scrolls. Reports refer to cards by title/list, with technical IDs shown only as secondary reference data. Choose a card in Referenced cards to open it directly. Actions that can change a board return proposed changes in a review window; select only the changes you want, then apply them explicitly. Archiving receives an additional confirmation. Each custom Board Action chooses Report or Propose changes and, for change actions, exactly which operations it may propose: create cards, update titles, append content, add existing labels, set dates, move cards, or archive cards. Signboard validates every selected operation against the current board again before writing it. Board content is sent to the selected provider only when you run an action; very large boards use a bounded context and report how many cards were omitted. Because whole-board analysis is substantially heavier than a card action, Ollama Board Actions may run for up to five minutes before Signboard times them out.

Custom Card and Board Actions can be exported as `.signboard-action` files, imported from the matching tab, or copied as share links. Share links use `cdevroe.com/signboard/smart-actions/` and keep the action package after `#` in the URL, so the website can preview and offer the file without receiving or storing the prompt on its server. As with any imported prompt, read its text and Board Action permissions before installing it.
