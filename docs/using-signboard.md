...
Custom Card and Board Actions can be exported as `.signboard-action` files, imported from the matching tab, or copied as share links. Share links use `cdevroe.com/signboard/actions/` and keep the action package after `#` in the URL, so the website can preview and offer the file without receiving or storing the prompt on its server. As with any imported prompt, read its text and Board Action permissions before installing it.
