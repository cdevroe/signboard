...
- Signboard MCP includes board-name resolution (`signboard_resolve_board_by_name`), Trello/Obsidian import tools, and supports both header-framed + newline-delimited stdio JSON-RPC.
...
- The terminal CLI now exposes `signboard import trello --file ...` and `signboard import obsidian --source ...`; MCP mirrors that with `signboard_import_trello` and `signboard_import_obsidian`.
